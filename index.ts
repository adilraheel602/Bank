...
class Customer{
    ...
    constructor(fName:string,lName:string,gender:string, age:number, contact:string){
        this.fName = fName;
        this.lName = lName;
        this.gender = gender;
        this.age = age;
...
class BankAccount   {
    debit=(debitAmount: number,customer:Customer)=>{    //if debit amount is less than balance then it will cancel transaction with a message.
    ...
    credit = (creditAmount:number, customer:Customer)=>{ //1$ will be deducted if more than 100$ is credited.
        if(creditAmount>=100){
            creditAmount -=1;
